...
int scale3;     /* Number of opposite bits to output after */
...
void done_encoding()
{   
    scale3 += 1;                       /* Output two bits that      */
    if ((l & MAX_VALUE | 0x7fff)==0x7fff && (l & 0x4000)==0x4000 ) bit_plus_foll(0);     /* select the quarter that   */
    else bit_plus_foll(1);                   /* the current code range    */
}                                              /* contains.                 */


void bit_plus_foll(int bit)
{  
    output_bit(bit);                           /* Output the bit.           */
    while (scale3>0) {
        output_bit(!bit);                      /* Output scale3     */
        scale3 -= 1;                   /* opposite bits. Set        */
    }                                          /* scale3 to zero.   */
}
